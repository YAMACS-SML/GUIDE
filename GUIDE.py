...
##storing mopac path
  ...
  if os.path.isfile(plgpath+'/'+'mopacpath.txt') :
    ...
    path= fk.readlines()
    mop= str((path[0]).strip('\n'))
    mopac=str((mop)+'/'+'MOPAC2016.exe')
    print(mopac)  
...
####MOPAC options
  ...
  if methodology == 'Single-point':
    ...
    nonelist =\
      ...
      "TextInput", 20, 48,"Insert the name of the file with extension",150,150,   
      "Button",      150,200," O K")
...
##counting the object present in the yasara window
  rcharge=open(macrotarget+'/'+'charge.log','w+')
  rcharge.write((str(alllist)).replace("'","").replace(" ", "\n").replace("[","").replace("]","").replace(",",""))
  rcharge.close()
  react_charge = open(macrotarget+'/'+'charge.log', "r")
...
if method == 'ORCA' and methodology == str(1) or methodology == str(2) or methodology == str(3) or methodology == str(4) or methodology ==str(5) or methodology ==str(6) or methodology ==str(7) or methodology ==str(8) or methodology ==str(10):
  resultlist =\
...
if method == 'ORCA' and methodology == str(3) or methodology == str(8) or methodology == str(9):  
  ...
  imgalllist =\
      yasara.ShowWin("Custom","No of Images",400,250,
      "NumberInput", 20, 88,"Image",8,8,100,
      "Button",      150,200," O K")  
##counting the object present in the yasara window
  rimg=open(macrotarget+'/'+'images.log','w+')
  rimg.write((str(imgalllist)).replace("'","").replace(" ", "\n").replace("[","").replace("]","").replace(",",""))
  rimg.close()
  img = open(macrotarget+'/'+'images.log', "r")
  noimg= img.readlines()
...
##Calculating the charge of the product
    ...
    promod=open(macrotarget+'/'+'procharge.log','w')
    promod.write(str(prochargeinfo).replace('Summed up net charge is ','').replace('[','').replace(']',''))
    promod.close()
    prochargef= open(macrotarget+'/'+'procharge.log','r')
    ...
    proalllist =\
      ...
      "Button",      150,200," O K")  
##counting the object present in the yasara window
    pcharge=open(macrotarget+'/'+'procharge.log','w+')
    pcharge.write((str(proalllist)).replace("'","").replace(" ", "\n").replace("[","").replace("]","").replace(",",""))
    pcharge.close()
    pro_charge = open(macrotarget+'/'+'procharge.log', "r")
...
###QM-MM region########################################################## 
    ...
    yasara.run("NameAtom Selected,L")
    #yasara.run("NameAtom all and not selected, con")
    yasara.ShowMessage("Please additionally select atoms for restraining")
    yasara.run("wait continuebutton")
    #yasara.run("UnselectAll")
    #yasara.run("SelectAtom all with distance < 1 from L")
    ####have to check
...
#####################################################################################
  ...
  if os.path.isfile(macrotarget+'/'+'ts_MEP_trj.xyz') :
    ...
    yasara.ShowMessage("Transition state calculation failed. Please check ts.out file for more information ") 
######## for single point calculation
  ...
  ###charge of the  molecule
    chargeinfo=yasara.run('ChargeObj all')
    yasara.run("SaveXYZ 1,"+str(macrotarget)+'/'+"inputmolecule.xyz,transform=Yes")
    cmod=open(macrotarget+'/'+'charge.log','w')
...
####temposection
    ...
    yasara.ShowMessage("Please select your molecule by selectbox and then click continue")
    yasara.run("wait continuebutton")
    yasara.run('ForceField AMBER03,SetPar=Yes')
  ###charge of the  molecule
    chargeinfo=yasara.run('ChargeObj selected')
    yasara.run("SaveXYZ Selected,"+str(macrotarget)+'/'+"inputmolecule.xyz,transform=Yes")
    cmod=open(macrotarget+'/'+'charge.log','w')
    cmod.write(str(chargeinfo).replace('Summed up net charge is ','').replace('[','').replace(']',''))
    cmod.close()
    chargef= open(macrotarget+'/'+'charge.log','r')
  ...
  alllist =\
    ...
    "Button",      150,200," O K")  
##counting the object present in the yasara window
  charge=open(macrotarget+'/'+'charge.log','w+')
  ...
  reactmultiplicity= str((reactinfo[1]).strip('\n'))
##modifying the reactant.xyz file without hamparing the main file
  with open(str(macrotarget)+'/'+'inputmolecule.xyz', 'r') as fin:
      data = fin.read().splitlines(True)
  with open(str(macrotarget)+'/'+'inputmolecule.txt', 'w') as fout:
      fout.writelines(data[2:]) 
      fout.close()  
##for methodology single point charge and homo-lumo difference calculation
  if methodology == str(1) or  methodology ==str(6):
    orcamolinp=open((macrotarget)+'/'+'inputmolecule.txt')
    moldata=orcamolinp.read()
    orcamolinp.close()
    orca_sp=open((macrotarget)+'/'+'inputmolecule.inp','w+')
    orca_sp.write("! SP ")
   ...
   # orca_sp.write('\nend')
    ...
    if mod == str(1):
      os.system(orca+' '+macrotarget+'/'+'inputmolecule.inp >  '+macrotarget+'/'+'inputmolecule.out')
    else:
      os.system('orca '+macrotarget+'/'+'inputmolecule.inp >  '+macrotarget+'/'+'inputmolecule.out')
    ...
    if methodology == str(1):
      scfline= open(macrotarget+'/'+'inputmolecule_property.txt')
      for line in scfline:
          if "SCF Energy:" in line:
             ...
             ####temposection
             gi=open(str(macrotarget)+'/'+'inputmolecule.xyz', 'r') 
             gidata=gi.readlines()
             ...
             with open(str(macrotarget)+'/'+'inputmolecule.txt', 'w') as fout:
                 fout.writelines(data[2:]) 
                 fout.close() 
             fi=open(str(macrotarget)+'/'+'inputmolecule.txt')
             fidata=fi.read()
             fo=open(str(macrotarget)+'/'+'finalstructure.xyz','w+')
             fo.write(str(fline))
             fo.write('\n')
             fo.write(str(secine))
             fo.write(' ')
        ...
        print('check')
        with open(macrotarget+"/"+'inputmolecule.out',"r") as fin, open(macrotarget+'/'+'orbital_energy.txt',"w") as fout:
        ...
        final= open (macrotarget+'/'+'orbital_energy.txt',"r+")
        finallines= final.readlines()[4:]
        final.seek(0)
        final.truncate()
        final.writelines(finallines)
        final.close()
        f = open(macrotarget+'/'+'orbital_energy.txt', "r")
        g = open(macrotarget+'/'+'orbital_energy_dat.txt', "w")
        for line in f:
          if line.strip():
        ...
        bad_words = ['0.0000']
        with open(macrotarget+'/'+'orbital_energy_dat.txt') as oldfile, open(macrotarget+'/'+'HOMO_dat.txt', 'w') as newfile:
             for line in oldfile:
                 if not any(bad_word in line for bad_word in bad_words):
                     newfile.write(line)

        fh = open(macrotarget+'/'+'HOMO_dat.txt', "r")
        gh = open(macrotarget+'/'+'HOMO_ev.txt', "w")
        for line in fh:
          if line.strip():
        ...
        with open(macrotarget+'/'+'HOMO_ev.txt', 'r') as fp:
            for count, line in enumerate(fp):
                pass
        homoline=int(count)
        lumoline=int(count+1)
        f=open(macrotarget+'/'+'orbital_energy_dat.txt')
        lines=f.readlines()
        homo=(lines[homoline])
        lumo=(lines[lumoline])
        energy=open(macrotarget+'/'+'HOMO_lumo_dat.txt', "w")
        energy.write(str(homo))
        energy.write('\n')
        energy.write(str(lumo))
        energy.close()
        fe = open(macrotarget+'/'+'HOMO_lumo_dat.txt', "r")
        ge = open(macrotarget+'/'+'HOMO_lumo_ev.txt', "w")
        for line in fe:
          if line.strip():
        ...
        fe.close()
        ge.close() 
        with open(macrotarget+'/'+'HOMO_lumo_ev.txt') as g:
        ...
        hlgap=round(hlgap,2)
        hlgap=str(hlgap)
        os.remove(macrotarget+'/'+'HOMO_lumo_dat.txt')
        #os.remove(macrotarget+'/'+'orbital_energy_dat.txt')
        os.remove(macrotarget+'/'+'HOMO_dat.txt')
        os.remove(macrotarget+'/'+'HOMO_ev.txt')
        os.remove(macrotarget+'/'+'orbital_energy.txt')
        yasara.ShowMessage("HOMO LUMO energy difference is "+ str(hlgap)+" eV") 
##for methodology geometry optimization
  elif methodology == str(2) or methodology == str(10):  
    orcamolinp=open((macrotarget)+'/'+'inputmolecule.txt')
    moldata=orcamolinp.read()
    orcamolinp.close()
    orca_geo=open((macrotarget)+'/'+'inputmolecule.inp','w+')
    orca_geo.write("! ")
...
##geometry optimixation of cation
      orca_cat=open((macrotarget)+'/'+'cation.inp','w+')
      orca_cat.write("! ")
      ...
      if mod == str(1):
        os.system(orca+' '+macrotarget+'/'+'cation.inp >  '+macrotarget+'/'+'cation.out')
      else:
        os.system('orca '+macrotarget+'/'+'cation.inp >  '+macrotarget+'/'+'cation.out')

##geometry optimixation of anion
      orca_anion=open((macrotarget)+'/'+'anion.inp','w+')
      orca_anion.write("! ")
      ...
      if mod == str(1):
        os.system(orca+' '+macrotarget+'/'+'anion.inp >  '+macrotarget+'/'+'anion.out')
      else:
        os.system('orca '+macrotarget+'/'+'anion.inp >  '+macrotarget+'/'+'anion.out')
      with open(macrotarget+'/'+'anion.out',"r") as fin, open(macrotarget+'/'+'hirshfeld_charge_anion.txt',"w") as fout:
      ...
      with open(macrotarget+'/'+'cation.out',"r") as fin, open(macrotarget+'/'+'hirshfeld_charge_cation.txt',"w") as fout:
           string = 'HIRSHFELD ANALYSIS'
           for line in fin:
               if string in line:
                  fout.write(line)
      ...
      with open(macrotarget+'/'+'hirshfeld_charge_anion.txt', 'r') as fin:
          data = fin.read().splitlines(True)
      with open(macrotarget+'/'+'hirshfeld_charge_anion_filter.txt',"w") as fout:
         fout.write("checkanion\n")
         fout.writelines(data[1:])

      with open(macrotarget+'/'+'hirshfeld_charge_anion_filter.txt',"r") as fin, open(macrotarget+'/'+'hirshfeld_charge_anion_final.txt',"w") as fout:
           string = 'HIRSHFELD ANALYSIS'
           for line in fin:
               if string in line:
                  fout.write(line)
                  try: 
                     while 'TIMINGS' not in line:
                      ...
                      pass  # ran out of file to read
      os.remove(macrotarget+'/'+'hirshfeld_charge_anion_filter.txt')
      with open(macrotarget+'/'+'hirshfeld_charge_cation.txt', 'r') as fin:
          data = fin.read().splitlines(True)
      with open(macrotarget+'/'+'hirshfeld_charge_cation_filter.txt',"w") as fout:
         fout.write("checkcation\n")
         fout.writelines(data[1:])

      with open(macrotarget+'/'+'hirshfeld_charge_cation_filter.txt',"r") as fin, open(macrotarget+'/'+'hirshfeld_charge_cation_final.txt',"w") as fout:
           string = 'HIRSHFELD ANALYSIS'
           for line in fin:
               if string in line:
                  fout.write(line)
                  try: 
                     while 'TIMINGS' not in line:
                      ...
                      pass  # ran out of file to read
      os.remove(macrotarget+'/'+'hirshfeld_charge_cation_filter.txt')
      with open(macrotarget+'/'+'hirshfeld_charge_cation_final.txt', 'r') as fin:
          data = fin.read().splitlines(True)
      with open(macrotarget+'/'+'cation_charge.txt',"w") as fout:
         fout.writelines(data[7:-5])
      with open(macrotarget+'/'+'hirshfeld_charge_anion_final.txt', 'r') as fin:
          data = fin.read().splitlines(True)
      with open(macrotarget+'/'+'anion_charge.txt',"w") as fout:
         fout.writelines(data[7:-5])
      os.remove(macrotarget+'/'+'hirshfeld_charge_cation_final.txt')
      os.remove(macrotarget+'/'+'hirshfeld_charge_anion_final.txt')
      os.remove(macrotarget+'/'+'hirshfeld_charge_anion.txt')
      os.remove(macrotarget+'/'+'hirshfeld_charge_cation.txt')

      f = open(macrotarget+'/'+'anion_charge.txt', "r")
      g = open(macrotarget+'/'+'anion_hirshfeld_charge.txt', "w")
      for line in f:
          if line.strip():
      ...
      k = open(macrotarget+'/'+'cation_charge.txt', "r")
      l = open(macrotarget+'/'+'cation_hirshfeld_charge.txt', "w")
      for line in k:
          if line.strip():
      ...
      m = open(macrotarget+'/'+'anion_charge.txt', "r")
      n = open(macrotarget+'/'+'atomname.txt', "w")
      for line in m:
          if line.strip():
      ...
      m.close()
      n.close()
      with open(macrotarget+'/'+'cation_hirshfeld_charge.txt', 'r') as fp:
          for count, line in enumerate(fp):
              pass
      i=0
      f= (count+1)
      #f=(loop)
      for i in range(0,f):
         read=open(macrotarget+'/'+'cation_hirshfeld_charge.txt','r')
         ...
         readanion=open(macrotarget+'/'+'anion_hirshfeld_charge.txt','r')
         ...
         if i == 0:
           val= open(macrotarget+'/'+'condensed_fukui.txt','w')
           val.write(str(fukuifunction).replace('[','').replace(']',''))
           val.close()
         else:
           valread=open(macrotarget+'/'+'condensed_fukui.txt','r')
           value=valread.readlines()
           valread.close()
           rewrite=open(macrotarget+'/'+'condensed_fukui.txt','w')
           rewrite.writelines(value)
           rewrite.write('\n')
      ...
      with open((macrotarget)+'/'+'condensed_fukui.txt') as xh:
        with open((macrotarget)+'/'+'atomname.txt') as yh:
          with open((macrotarget)+'/'+'fukuifunction_results.txt',"w") as zh:
       #Read first file
             xlines = xh.readlines()
         #Read second file
             ylines = yh.readlines()
...
##for meethodology vibrational frequency
    ...
    if os.path.isfile(macrotarget+'/'+'inputmolecule.hess'):
      ...
      if a == str(1):
        if mod == str(1):
          os.rename(macrotarget+'/'+'inputmolecule.trj', macrotarget+'/'+'trajectory.xyz' )
        else:
          os.rename(macrotarget+'/'+'inputmolecule_trj.xyz', macrotarget+'/'+'trajectory.xyz' )
        yasara.run('DelObj all')  
        yasara.run('Loadxyz '+macrotarget+'/'+'trajectory.xyz')
        yasara.run('ZoomAll Steps=20')
        obj=yasara.run("CountObj all")
        deltext= open(macrotarget+'/'+'deltext.txt','w')
        deltext.write(str(obj).replace('object(s) match the selection.','').replace('[','').replace(']',''))
...
###for methodology uv spectroscopy
    ...
    moldata=orcamolinp.read()
    orcamolinp.close()
    orca_uv=open((macrotarget)+'/'+'inputmolecule.inp','w+')
    orca_uv.write("! ")
    ...
    if mod == str(1):
      ...
      os.chdir(macrotarget)
      os.system('./orca_mapspc '+macrotarget+'/'+'inputmolecule.out ABS -W1000' )
    ...
    file_path = macrotarget+'/'+'inputmolecule.out.abs.dat'
    dataframe1 = pd.read_csv(file_path, delim_whitespace=True)

    dataframe1.to_csv((macrotarget)+'/'+'inputmolecule.csv', index = None)
    df = pd.read_csv((macrotarget)+'/'+'inputmolecule.csv', header=None)
    ...
    dr=pd.DataFrame(hr)
    dr.to_csv((macrotarget)+'/'+'uvdata.csv', index = None)




    with open((macrotarget)+'/'+'uvdata.csv', 'r+') as f:
      headers = f.readline()
      firstData = f.readline()
      f.seek(0)
      firstData = firstData[:-1] + ' ' * len(headers) + '\n'
      f.write(firstData)
    with open((macrotarget)+'/'+'uvdata.csv', 'r+') as fd:
        lines = fd.readlines()
        fd.seek(0)
    ...
    with open((macrotarget)+'/'+'uvdata.csv','r') as csvfile:
...
#plt.grid()
    plt.savefig((macrotarget)+'/'+'uvdata.png')     
    thirdpng='LoadPNG '+(macrotarget)+'/'+'uvdata.png'
    yasara.run(thirdpng)
...
###object selection  
  ...
  g.close()
  if noobj== str(0):
    if methodology == 'None' and os.path.isfile(macrotarget+'/'+ownfile):
      ...
      yasara.run("SaveXYZ 1,"+str(macrotarget)+'/'+"inputmolecule.xyz,transform=Yes")
      yasara.run('DelObj all')  
      yasara.run('Loadxyz '+(macrotarget)+'/'+'inputmolecule.xyz')
...
##homo-lumo energy gap calculation    
  if methodology == 'HOMO-LUMO':  
    keys="DENSITY"
    yasara.run('LogAs '+(str(macrotarget))+'/'+'qm_log,Append=No,RunMOPACObj 1,'+str(keys))    
    with open(macrotarget+'/'+'qm_log.log',"r") as fin, open(macrotarget+'/'+'qm_first.txt',"w") as fout:
         string = 'EIGENVALUES'
         for line in fin:
             if string in line:
                fout.write(line)
    ...
    with open(macrotarget+'/'+'qm_first.txt',"r") as fin, open(macrotarget+'/'+'EIGENVALUES.txt',"w") as fout:
         string = 'EIGENVALUES'
         for line in fin:
             if string in line:
                fout.write(line)
                try: 
                   while 'NET ATOMIC CHARGES AND DIPOLE CONTRIBUTIONS' not in line:
                    ...
                    pass  # ran out of file to read                
    remove_words=['NET']
    with open(macrotarget+'/'+'EIGENVALUES.txt') as oldfile, open(macrotarget+'/'+'EIGENVALUES_filter.txt', 'w') as newfile:
        for line in oldfile:
            if not any(remove_word in line for remove_word in remove_words):
                newfile.write((line).replace('EIGENVALUES',''))

    os.remove(macrotarget+'/'+'EIGENVALUES.txt')
    with open(macrotarget+'/'+'EIGENVALUES_filter.txt') as reader, open(macrotarget+'/'+'EIGENVALUES_filter.txt', 'r+') as writer:
      for line in reader:
        if line.strip():
          writer.write((line).replace('\n',''))
      writer.truncate()

    with open(macrotarget+'/'+'qm_log.log',"r") as fin, open(macrotarget+'/'+'qm_value.txt',"w") as fout:
             ...
             if string in line:
                ...
                try: 
                   while 'NO. OF FILLED LEVELS' not in line:
                    ...
                    pass  # ran out of file to read
    with open (macrotarget+'/'+'qm_value.txt',"r") as f:
        data=f.readlines()
        p=((data[1]).strip('\n').strip('          '))
        print(p)
        f.close()
    ...
    eigenarr = np.genfromtxt(macrotarget+'/'+'EIGENVALUES_filter.txt')
  ...
  #x=eigenarr[eigenarr < 0]
    ...
    lumo = round(a,3)
    homo = round(b,3)
    os.remove(macrotarget+'/'+'EIGENVALUES_filter.txt')
    yasara.ShowMessage(" HOMO " + (str(homo)) + " LUMO " + (str(lumo)) + " gap: " + (str(homolumodiff))+ " eV")

  elif methodology == 'Single-point': 

      fh = open(macrotarget+'/'+'inputmolecule.xyz', "r")
      gh = open(macrotarget+'/'+'inputmolecule_xyz.txt', "w")
      for line in fh:
         if line.strip():
      ...
      fh.close()
      gh.close()
      x= open(macrotarget+'/'+'inputmolecule_xyz.txt','r')
      data=x.read()

      y=open(macrotarget+'/'+'inputmolecule_xyz.txt','w+')
      ...
      with open(macrotarget+'/'+'inputmolecule_xyz.txt') as xh:
        with open(macrotarget+'/'+'atomname.txt') as yh:
          with open(macrotarget+'/'+'inputmolecule.txt',"w") as zh:
             #Read first file
             xlines = xh.readlines()
               #Read second file
             ylines = yh.readlines()
      ...
      with open(macrotarget+'/'+'inputmolecule.txt', 'r') as fin:
         data = fin.read().splitlines(True)
      with open(macrotarget+'/'+'inputmolecule.txt', 'w') as fout:
         fout.writelines(data[1:])
         fout.close()
      f = open((macrotarget)+'/'+'inputmolecule.txt', "r")
      molcontent= f.read()
      f.close()
      inp=open(macrotarget+'/'+'inputmolecule.mop', 'w+')  
      inp.write(" ")
      ...
      os.system(mopac+' '+macrotarget+'/'+'inputmolecule.mop')
      errorline= open(macrotarget+'/'+'inputmolecule.out')
